    ...
    andThen(f) {
        return new ResultAsync(
            this._promise.then((res) => {
                ...
                return newValue instanceof ResultAsync
                    ? newValue._promise
                    : newValue;
            })
        );
            ...
            return yield __await(
                yield __await(
                    yield* __asyncDelegator(
                        __asyncValues(
                            yield __await(
                                this._promise.then((res) => res.safeUnwrap())
                            )
                        )
                    )
                )
 ...
 */
const combineResultAsyncList = (asyncResultList) =>
...
const combineResultAsyncListWithAllErrors = (asyncResultList) =>
    ResultAsync.fromSafePromise(Promise.all(asyncResultList)).andThen(
