...
export const createGameMachine = setup({
    ...
    actions: {
        startGame: assign({
            ...
            winner: () => null,
            aiManager: ({ context }) =>
                context.aiManager || new AIManager(context.config),
        }),
        ...
        playCard: assign(({ context, event }) => {
            ...
            emitGameEvent(GameEventType.CARD_PLAYED, {
                playerId,
                cardId,
                card,
            });
        ...
        drawCard: assign(({ context, event }) => {
            ...
            emitGameEvent(GameEventType.CARD_DRAWN, {
                playerId,
                card: drawnCard,
            });
        ...
        nextPlayer: assign(({ context }) => {
            ...
            else if (topCard.type === UnoCardType.REVERSE)
                directionChange = true;
            const newDirection = directionChange
                ? -context.direction
                : context.direction;
        ...
        setWinner: assign(({ context }) => {
            ...
            if (winner) {
                emitGameEvent(GameEventType.GAME_OVER, {
                    winnerId: winner.id,
    ...
    states: {
        ...
        checkingWin: {
            always: [
                {
                    target: 'gameOver',
                    cond: (context: GameMachineContext) =>
                        context.winner !== null,
                },
                { target: 'playing' },
            ],
